...
const FdPizzaBasicSelect: FC<FdPizzaBasicSelectProps> = (client) => {
  ...
  const mappingItems = (type: string, subType?: string, filter?: any) => {
      ...
      /* if(type == "pizza")
        return itemsFile.items.filter(f => f.code == client.client.code)[0][type].map( i => { console.log(type);
          return <tr className="align-middle">
            <td>
              <Form.Check
                type="radio"
                label=""
                name={ type == "pizza" ? "Pizza" : "Pizza-2" }
                id={ i.name }
                style={{ marginLeft: "10px" }}
                onClick={ () => { orderAddProductValue(i.name ?? '', i.price ?? 0); type == "pizza" ? productChangeHalf('', 0) : productChangeHalf(i.name ?? '', i.price ?? 0) } }
                checked={ type == "pizza" ? sList[index].name == i.name : sList[index].half == i.name }
              />
            </td>
            <td>
              <Figure.Image
  ...
  return (
    <Container style={{ marginTop: "1.5rem", marginBottom: "1.5rem" }}>
      ...
      {/* <p>{ sList2.map(m => { return <h1>{ m.name }</h1> }) }</p> */}
